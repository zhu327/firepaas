...
var routeScope = map[string]string{
	...
	// ADR-0040 §15 G2a：EastWest 规则直接作用于数据面（放行 = 授权），
	// 按写权限收口；GET 也取 write（策略表是攻击面侦察信息）。
	"PUT /v1/eastwest-policies":      "write",
	"GET /v1/eastwest-policies":      "write",
	"DELETE /v1/eastwest-policies":   "write",
	"PUT /v1/apps/{id}/secret-refs":  "write",
	"POST /v1/apps":                  "deploy",
	"POST /v1/apps/{id}/deployments": "deploy",
	"POST /v1/apps/{id}/scale":       "deploy",
	// ADR-0041：策略写 = deploy（与 scale 同级），读 = read。
	"PUT /v1/apps/{id}/autoscale":   "deploy",
	"GET /v1/apps/{id}/autoscale":   "read",
...
// requireGlobalIdentity 是平台专属路由的 handler 层复核（review 2026-09-10）：
// `admin` capability 同时被项目 owner（项目内 admin）持有，中间件无法区分
// “项目 admin”与“平台 admin”。节点运维、投影重建、项目配额/限流写属于
// 集群级能力，必须要求全局身份；非全局身份返回 403，fail closed。
//
// 调用点必须位于任何副作用之前。中间件的 admin scope 门槛仍保留（双保险）。
func (a *API) requireGlobalIdentity(w http.ResponseWriter, r *http.Request) bool {
	if identityIsGlobal(identFrom(r)) {
		return true
	}
	writeErr(w, 403, "global identity required (root token or unscoped key)")
	return false
}
...
var projectGated = map[string]bool{
	...
	"PUT /v1/apps/{id}/autoscale":                            true,
	"GET /v1/apps/{id}/autoscale":                            true,
