...
func (a *API) putProjectQuota(w http.ResponseWriter, r *http.Request) {
	// review 2026-09-10：配额写是平台治理动作；受限 project admin 不得
	// 自行抬高配额（routeScope 要求 admin，但 owner 也持有 admin）。
...
// getRateLimits 保留给项目内 admin 读自己的配置（projectGated 已限定只能访问
// 自己 project；review L5：受限身份不应因平台写权限收紧而丢失读取可见性）。
