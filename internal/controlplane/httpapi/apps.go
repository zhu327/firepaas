...
func (a *API) scaleApp(w http.ResponseWriter, r *http.Request) {
	...
	// ADR-0041 §1/§4.8：手动 scale 一律走接管语义（P1-2 TOCTOU 修复）——
	// 单条 UPDATE 写 desired_replicas 且置 enabled=false（store.TakeoverScale，
	// 对未启用 app 幂等）。禁止“先读 enabled 再分支”：GetApp 与写入之间的
	// 并发 PUT enable 会让 SetAppReplicas 路径漏掉接管，下一拍被 autoscaler
	// CAS 覆盖。GetApp 仅用于 404 与审计 prev 值。
	app, err := a.store.GetApp(r.Context(), appID)
	if err != nil {
		writeInternalErr(w, r, err)
		return
	...
	if !app.Deleted {
		...
		a.enqueueUserEvent(app.ProjectID, appID, "", store.UserEventAutoscaleTakeover, map[string]any{
	...
	// 仅已删除 app 可达（TakeoverScale 与 SetAppReplicas 均拒绝墓碑；
	// ErrAppDeleted 保持历史 500 映射，本次不改变）。
...
func marshalEgressPolicy(b *egressPolicyBody, generation int64) (json.RawMessage, error) {
	...
	if err := agentv1.ValidateEgressPolicySubmission(&pb.EgressPolicySpec{
		Mode:              mode,
		AllowedCidrs:      b.AllowedCIDRs,
		DeniedCidrs:       b.DeniedCIDRs,
		AllowedDomains:    domains,
